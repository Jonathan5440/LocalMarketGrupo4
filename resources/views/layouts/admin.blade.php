...
<head>
    ...
    <style>
        .sidebar-link{display:flex;align-items:center;gap:10px;padding:10px 16px;border-radius:10px;font-size:14px;font-weight:500;color:#c7d2fe;transition:all .18s}
        .sidebar-link:hover,.sidebar-link.active{background:rgba(255,255,255,.12);color:#fff}
        .sidebar-link.active{background:rgba(255,255,255,.18);box-shadow:inset 3px 0 0 #a5b4fc}
        .stat-card{background:#fff;border-radius:16px;padding:24px;box-shadow:0 1px 3px rgba(0,0,0,.08),0 4px 16px rgba(0,0,0,.04);transition:transform .2s,box-shadow .2s}
        .stat-card:hover{transform:translateY(-2px);box-shadow:0 4px 24px rgba(0,0,0,.10)}
    </style>
</head>
...
<div class="flex min-h-screen">
    ...
    <div class="ml-64 flex-1 flex flex-col min-h-screen">
            ...
            @if(session('success'))
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        Swal.fire({ icon:'success', title:'¡Listo!', text:"{{ session('success') }}", timer:2500, showConfirmButton:false, toast:true, position:'top-end' });
                    });
                </script>
            @endif
            @if(session('error'))
        ...
        <footer class="px-6 py-4 border-t border-slate-200 bg-white">
            <p class="text-slate-400 text-xs text-center">LocalMarket © {{ date('Y') }} · Grupo #4 · Panel Administrativo</p>
        </footer>
    </div>
</div>
